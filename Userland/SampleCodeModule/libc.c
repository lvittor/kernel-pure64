...
int print_f(fd_t fd, const char *format, ...)
{
	...
	for (traverse = format; *traverse != '\0'; traverse++) {
		...
		case 'g':
			...
			i = (uint64_t)(
				g *
				1000); // cantidad de decimales significativos (1000 -> 3 decimales significativos)
			if (uintToBase(i, buff, 10) > 0)
				put_s(fd, buff);
			break;
		case '\0':
...
int sscan(char *buff, char *fmt, ...)
{
	...
	while (fmt && fmt[i]) {
		if (fmt[i] == '%') {
			...
			case 'd': {
				*(int64_t *)va_arg(vl, int64_t *) =
					strtoint(&buff[j], &out_loc, 10);
				j += out_loc - &buff[j];
				ret++;
				break;
			}
