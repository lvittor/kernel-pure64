...
#define MAX_PROCESSES                                                          \
	102 //Should be around 80% of the the processes handled by the kernel
...
void test_processes(void)
{
	...
	while (1) {
		...
		for (rq = 0; rq < MAX_PROCESSES; rq++) {
			p_rqs[rq].pid = my_create_process(
				"endless_loop"); // TODO: Port this call as required

			if (p_rqs[rq].pid ==
			    -1) { // TODO: Port this as required
