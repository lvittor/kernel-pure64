...
{
	uint8_t *screen =
		(uint8_t *)((uint64_t)vbeInfo->framebuffer +
			    x * vbeInfo->bpp / 8 + (int)y * vbeInfo->pitch);
	screen[0] = color->b;
	screen[1] = color->g;
	screen[2] = color->r;
}
...
static void scrollUp(Color *backgroundColor)
{
	for (int dy = 0; dy < prompt.windowHeight - font.Height; dy++) {
		...
		uint8_t *to = (uint8_t *)((uint64_t)vbeInfo->framebuffer +
					  x * vbeInfo->bpp / 8 +
					  (int)yTo * vbeInfo->pitch);
		uint8_t *from = (uint8_t *)((uint64_t)vbeInfo->framebuffer +
					    x * vbeInfo->bpp / 8 +
					    (int)yFrom * vbeInfo->pitch);
		memcpy(to, from, prompt.windowWidth * vbeInfo->bpp / 8);
	}

	for (int y = prompt.windowHeight - font.Height; y < prompt.windowHeight;
	     y++)
		for (int x = prompt.baseX;
		     x < prompt.baseX + prompt.windowWidth; x++)
...
void clearWindow(Color *backgroundColor)
{
	for (int x = 0; x < prompt.windowWidth; x++)
		for (int y = 0; y < prompt.y * font.Height; y++)
			setPixel(prompt.baseX + x, prompt.baseY + y,
				 backgroundColor);
	...
	for (int i = 0; i < font.Height; i++) {
		for (int j = 0; j < font.Width; j++) {
			if (bitMapCharacter[i] &
				...
				setPixel(baseXPixel + j, baseYPixel + i,
					 fontColor);
			else
				setPixel(baseXPixel + j, baseYPixel + i,
					 backgroundColor);
